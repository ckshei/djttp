App.eventsShow = {
  run: function() {
    $.get("/events/" + $(".js-event").data("id") + ".json", function(data) {
        var ev = new Event(data.id, data.name, data.date, data.playlist.spotify_url, data.playlist.sync_url, data.guest_count, data.event_url, data.playlist.song_count)
